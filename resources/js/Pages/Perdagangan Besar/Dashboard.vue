    ...
    <div class="min-h-screen bg-slate-950 text-slate-100 flex font-sans">
        ...
        <aside class="w-64 bg-slate-900 border-r border-slate-850 shrink-0 hidden md:flex flex-col justify-between p-6">
            <div class="space-y-8">
                ...
                <div class="flex items-center gap-3">
                    <div class="p-2 bg-amber-500/10 border border-amber-500/30 rounded-xl text-amber-405">
                        <span class="material-symbols-outlined font-bold">local_shipping</span>
                    </div>
                    <div>
                        <h2 class="text-xs font-black text-white leading-tight">Admin Dagang</h2>
                ...
                <!-- Nav -->
                <nav class="space-y-1">
                    <a href="#" class="flex items-center gap-3 px-4 py-3 bg-amber-500/10 text-amber-300 font-bold text-xs rounded-xl border border-amber-500/20">
            ...
            <div class="border-t border-slate-850 pt-5 space-y-4">
                <div class="flex items-center gap-3">
                    <div class="w-9 h-9 rounded-full bg-amber-500 text-slate-950 flex items-center justify-center font-bold text-sm">
                        {{ user.nama.charAt(0) }}
                    </div>
            ...
            <div class="p-6 space-y-6 max-w-6xl w-full mx-auto">
                ...
                <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
                    <div class="bg-slate-900 border border-slate-850 p-5 rounded-2xl space-y-3 shadow-md">
                        <div class="flex items-center justify-between">
                            <span class="text-[10px] font-bold text-slate-450 uppercase tracking-widest">Omset Aktif</span>
                            <span class="material-symbols-outlined text-amber-450 text-lg">monetization_on</span>
                        </div>
                        <div>
                            <h3 class="text-2xl font-black text-white">{{ totalTransactionVal }}</h3>
                    ...
                    <div class="bg-slate-900 border border-slate-850 p-5 rounded-2xl space-y-3 shadow-md">
                        <div class="flex items-center justify-between">
                            <span class="text-[10px] font-bold text-slate-450 uppercase tracking-widest">Kargo Aktif</span>
                            <span class="material-symbols-outlined text-amber-450 text-lg">local_shipping</span>
                        </div>
                        <div>
                            <h3 class="text-2xl font-black text-white">{{ orders.filter(o => o.status === 'Dalam Pengiriman').length }} Truk</h3>
                    ...
                    <div class="bg-slate-900 border border-slate-850 p-5 rounded-2xl space-y-3 shadow-md">
                        <div class="flex items-center justify-between">
                            <span class="text-[10px] font-bold text-slate-450 uppercase tracking-widest">Total Komoditas</span>
                            <span class="material-symbols-outlined text-amber-450 text-lg">inventory_2</span>
                        </div>
                        <div>
                            <h3 class="text-2xl font-black text-white">18,2 Ton</h3>
                    ...
                    <div class="bg-slate-900 border border-slate-850 p-5 rounded-2xl space-y-3 shadow-md">
                        <div class="flex items-center justify-between">
                            <span class="text-[10px] font-bold text-slate-450 uppercase tracking-widest">Mitra Korporasi</span>
                            <span class="material-symbols-outlined text-amber-450 text-lg">handshake</span>
                        </div>
                        <div>
                            <h3 class="text-2xl font-black text-white">18 Perusahaan</h3>
                ...
                <div class="bg-slate-900 border border-slate-850 rounded-2xl shadow-xl overflow-hidden">
                    <div class="px-6 py-5 border-b border-slate-850 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                        <div>
                            <h3 class="text-sm font-extrabold text-white uppercase tracking-wider">Kelola Transaksi Kontrak Grosir</h3>
                        ...
                        <div class="flex flex-wrap items-center gap-3">
                            <input v-model="search" type="text" placeholder="Cari perusahaan / komoditas" class="rounded-xl border border-slate-800 bg-slate-950 px-3.5 py-2 text-xs text-slate-200 placeholder-slate-600 focus:border-amber-500 focus:outline-none" />
                            
                            <select v-model="filterStatus" class="rounded-xl border border-slate-800 bg-slate-950 px-3 py-2 text-xs text-slate-200 focus:border-amber-500 focus:outline-none">
                            ...
                            <button @click="isAddModalOpen = true" class="inline-flex items-center gap-1 px-4 py-2 bg-amber-500 hover:bg-amber-400 text-slate-950 font-bold text-xs rounded-xl shadow-lg transition">
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-left border-collapse text-xs">
                            ...
                            <tbody class="divide-y divide-slate-850">
                                <tr v-for="order in filteredOrders" :key="order.id" class="hover:bg-slate-850/30 transition-colors">
                                    <td class="p-4">
                                        <p class="font-bold text-white">{{ order.company }}</p>
                                        <p class="text-[10px] text-slate-500 mt-0.5">{{ order.contact }} • {{ order.phone }}</p>
                                    </td>
                                    <td class="p-4 text-slate-300 font-semibold">{{ order.commodity }}</td>
                                    <td class="p-4 text-slate-405 font-bold">{{ order.volume }}</td>
                                    <td class="p-4 text-slate-200 font-black">{{ order.total }}</td>
                                    <td class="p-4 text-slate-400">{{ order.date }}</td>
                                    <td class="p-4">
                                        <span v-if="order.status === 'Selesai'" class="px-2.5 py-0.5 bg-emerald-950 text-emerald-450 border border-emerald-900/40 rounded-full text-[10px] font-extrabold uppercase">Selesai</span>
                                        <span v-else-if="order.status === 'Dalam Pengiriman'" class="px-2.5 py-0.5 bg-indigo-950 text-indigo-400 border border-indigo-900/40 rounded-full text-[10px] font-extrabold uppercase">Pengiriman</span>
                                        <span v-else-if="order.status === 'Menunggu Pembayaran'" class="px-2.5 py-0.5 bg-amber-950/50 text-amber-400 border border-amber-900/40 rounded-full text-[10px] font-extrabold uppercase">Menunggu</span>
                                    ...
                                    <td class="p-4 flex items-center justify-center gap-2">
                                        <div v-if="order.status === 'Menunggu Pembayaran'" class="flex gap-1.5">
                                            <button @click="updateStatus(order.id, 'Dalam Pengiriman')" class="px-2.5 py-1.5 bg-indigo-650 hover:bg-indigo-500 text-white rounded-lg text-[10px] font-bold transition">Kirim Kargo</button>
                                        ...
                                        <div v-else-if="order.status === 'Dalam Pengiriman'">
                                            <button @click="updateStatus(order.id, 'Selesai')" class="px-2.5 py-1.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-lg text-[10px] font-bold transition">Selesai Bongkar</button>
                                        </div>
        ...
        <div v-if="isAddModalOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm">
            <div class="w-full max-w-md bg-slate-900 border border-slate-800 rounded-3xl p-6 space-y-6 shadow-2xl relative">
                ...
                <h3 class="text-base font-extrabold text-white uppercase tracking-wider">Buat Kontrak Invoice Baru</h3>
                ...
                <form @submit.prevent="createOrder" class="space-y-4">
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            <label class="block text-[10px] font-bold text-slate-400 uppercase mb-1">Nama Perusahaan</label>
                            <input v-model="newCompany" type="text" placeholder="PT / CV..." class="w-full rounded-xl border border-slate-800 bg-slate-950 px-4 py-3 text-xs text-slate-200 focus:border-amber-500 focus:outline-none" required />
                        </div>
                        <div>
                            <label class="block text-[10px] font-bold text-slate-400 uppercase mb-1">Nama Perwakilan</label>
                            <input v-model="newContact" type="text" placeholder="Nama Person" class="w-full rounded-xl border border-slate-800 bg-slate-950 px-4 py-3 text-xs text-slate-200 focus:border-amber-500 focus:outline-none" required />
                        </div>
                    </div>
                    <div>
                        <label class="block text-[10px] font-bold text-slate-400 uppercase mb-1">Komoditas</label>
                        <select v-model="newCommodity" class="w-full rounded-xl border border-slate-800 bg-slate-950 px-4 py-3 text-xs text-slate-200 focus:border-amber-500 focus:outline-none">
                    ...
                    <div>
                        <label class="block text-[10px] font-bold text-slate-400 uppercase mb-1">Volume (kg / Liter)</label>
                        <input v-model="newVolume" type="number" min="1" class="w-full rounded-xl border border-slate-800 bg-slate-950 px-4 py-3 text-xs text-slate-200 focus:border-amber-500 focus:outline-none" required />
                    </div>
                    <div>
                        <label class="block text-[10px] font-bold text-slate-400 uppercase mb-1">Status Sesi</label>
                        <select v-model="newStatus" class="w-full rounded-xl border border-slate-800 bg-slate-950 px-4 py-3 text-xs text-slate-200 focus:border-amber-500 focus:outline-none">
                            <option value="Menunggu Pembayaran">Menunggu Pembayaran</option>
                            <option value="Dalam Pengiriman">Dalam Pengiriman</option>
                            <option value="Selesai">Selesai</option>
                        </select>
                    ...
                    <button type="submit" class="w-full py-3 bg-amber-500 hover:bg-amber-400 text-slate-955 font-bold text-xs rounded-xl transition">
