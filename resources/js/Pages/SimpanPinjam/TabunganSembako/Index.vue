...
function openEditTrx(trx) {
    ...
    editTrxUrl.value  = route('tabungan-sembako.transaksi.update', trx.id);
    showEdit.value    = true;
}

async function handleEditSaved() {
    showEdit.value = false;
    // Reload riwayat transaksi yang sedang terbuka
...
async function deleteTrx(trx) {
    ...
    try {
        const url = route('tabungan-sembako.transaksi.destroy', trx.id);
...
<template>
    ...
    <AuthenticatedLayout>
        ...
        <div class="space-y-5">
            ...
            <!-- Pagination -->
            <div v-if="nasabah.last_page > 1" class="flex items-center justify-between border-t border-[color:var(--color-outline-variant)] px-4 py-3 bg-white rounded-b-xl border-x border-b">
