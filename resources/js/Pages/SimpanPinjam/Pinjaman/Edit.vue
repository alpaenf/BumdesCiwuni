...
const props = defineProps({ pinjaman: Object, nasabahOptions: Array });

const form = useForm({
    nasabah_id:      props.pinjaman.nasabah_id,
    tanggal_akad:    props.pinjaman.tanggal_akad ? props.pinjaman.tanggal_akad.substring(0, 10) : '',
    jenis_pinjaman:  props.pinjaman.jenis_pinjaman || 'uang',
    keterangan:      props.pinjaman.keterangan || '',
    pinjaman_pokok:  props.pinjaman.pinjaman_pokok,
    bunga:           props.pinjaman.bunga,
    nominal_setoran: props.pinjaman.nominal_setoran,
    biaya_tambahan:  props.pinjaman.biaya_tambahan,
...
const submit = () => form.post(route('pinjaman.update', props.pinjaman.id));
</script>

<template>
    <Head title="Edit Pinjaman" />
    <AuthenticatedLayout>
        <template #header>Edit Pinjaman</template>
        <div class="mx-auto max-w-xl">
            <Link :href="route('pinjaman.show', pinjaman.id)" class="mb-6 inline-flex items-center gap-2 text-sm text-[color:var(--color-secondary)] hover:text-[color:var(--color-on-surface)]">
                <span class="material-symbols-outlined text-base">arrow_back</span> Kembali ke Detail Pinjaman
            </Link>
            <div class="rounded-xl border border-[color:var(--color-outline-variant)] bg-white p-6 shadow-sm">
                <h2 class="mb-6 font-semibold">Edit Data Pinjaman</h2>
                
                <div class="mb-4 rounded-lg bg-amber-50 border border-amber-200 p-3 text-sm text-amber-800">
                    Perhatian: Mengubah pinjaman pokok, bunga, atau biaya tambahan akan menghitung ulang sisa pinjaman dan seluruh riwayat angsurannya secara otomatis.
                </div>
                ...
                <form @submit.prevent="submit" class="space-y-4">
                    ...
                    <div>
                        ...
                        <select v-model="form.jenis_pinjaman" class="w-full rounded-lg border px-4 py-2.5 text-sm focus:outline-none focus:border-[color:var(--color-primary)] border-[color:var(--color-outline-variant)]">
                    ...
                    <div v-if="form.jenis_pinjaman !== 'uang'" class="bg-amber-50 border border-amber-200 p-4 rounded-xl space-y-4">
                        <div>
                            <label class="mb-1.5 block text-sm font-medium text-amber-900">Nama/Keterangan Barang <span class="text-red-500">*</span></label>
                            <input v-model="form.keterangan" type="text" placeholder="Misal: Sepeda Motor Honda Beat"
                        ...
                        <div>
                            <label class="mb-1.5 block text-sm font-medium text-amber-900">Foto Barang Baru (Opsional)</label>
                            <input
                                type="file"
                                accept="image/*"
                            ...
                            />
                            <p class="mt-1.5 text-xs text-amber-700">Abaikan jika tidak ingin mengubah foto. Format: JPG, JPEG, PNG.</p>
                    ...
                    <!-- Foto Perjanjian -->
                    <div>
                        <label class="mb-1.5 block text-sm font-medium">Foto Dokumen Perjanjian Baru (Opsional)</label>
                        <input
                            type="file"
                            accept="image/*"
                        ...
                        />
                        <p class="mt-1.5 text-xs text-[color:var(--color-secondary)]">Abaikan jika tidak ingin mengubah foto. Format: JPG, JPEG, PNG, GIF.</p>
                    ...
                    <div class="flex justify-end gap-3 pt-2">
                        <Link :href="route('pinjaman.show', pinjaman.id)" class="rounded-lg border border-[color:var(--color-outline-variant)] px-5 py-2.5 text-sm font-medium hover:bg-[color:var(--color-surface-container)]">Batal</Link>
