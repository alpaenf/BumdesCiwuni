...
watch(search, () => {
    ...
    searchTimeout = setTimeout(() => {
        router.get(route('tabungan.index'), { search: search.value }, { preserveState: true, replace: true });
    }, 400);
});
...
async function openStrukModal(nasabah) {
    ...
    try {
        const res = await fetch(route('tabungan.riwayat', nasabah.id));
...
function openStruk(transaksiId) {
    window.open(route('tabungan.struk', transaksiId), '_blank');
}
...
const editTrxData = ref(null);
const editTrxUrl  = ref('');
const csrfToken   = usePage().props.csrf_token
                  ?? document.querySelector('meta[name="csrf-token"]')?.content;

function openEditTrx(trx) {
    editTrxData.value = {
        id:              trx.id,
    ...
    editTrxUrl.value  = route('tabungan.transaksi.update', trx.id);
    showEdit.value    = true;
}
...
<template>
                    ...
                    <table class="w-full text-sm min-w-[800px]">
                        <thead>
                            <tr class="border-b border-[color:var(--color-outline-variant)] bg-[color:var(--color-surface-container-low)]">
                                ...
                                <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-[color:var(--color-secondary)]">Saldo</th>
            ...
            <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center p-4" @click.self="closeModal">
                ...
                <div class="relative z-10 w-full max-w-md rounded-2xl bg-white shadow-2xl">
                    ...
                    <div class="flex items-center justify-between border-b border-slate-100 px-5 py-4">
                        <div>
                            <h3 class="font-semibold text-slate-800">Cetak Struk Tabungan</h3>
                    ...
                    <div class="px-5 py-4 max-h-80 overflow-y-auto">
                        ...
                        <!-- Error -->
                        <div v-else-if="modalError" class="rounded-lg bg-red-50 px-4 py-3 text-sm text-red-600">
                            {{ modalError }}
                        </div>
                        ...
                        <ul v-else class="divide-y divide-slate-100">
                            <li v-for="trx in modalTransaksi" :key="trx.id"
    ...
    <EditTransaksiModal
        :show="showEdit"
        :transaksi="editTrxData"
        :update-url="editTrxUrl"
        mode="tabungan"
