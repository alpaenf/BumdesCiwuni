...
<template>
    <Head title="Unit Ketahanan Pangan" />
    ...
    <div class="min-h-screen bg-[#ffffff] text-[#181d18] font-sans antialiased selection:bg-blue-600 selection:text-white">
        ...
        <header :class="[
            ...
            <div class="flex items-center justify-between">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <div>
                        <h1 :class="['text-xs font-extrabold leading-tight transition-colors', isScrolled ? 'text-blue-800' : 'text-white']">Ketahanan Pangan</h1>
                ...
                <nav class="hidden md:flex items-center gap-5">
                    ...
                    <div :class="['border-l pl-4 flex items-center gap-2 transition-colors', isScrolled ? 'border-[#bfc9bd]' : 'border-white/30']">
                        ...
                        <template v-else>
                            <Link :href="route('unit.login', { slug: 'ketahanan-pangan' })" :class="['inline-flex items-center justify-center px-4 py-2 font-semibold text-xs rounded-lg transition-colors border', isScrolled ? 'border-blue-600 text-blue-700 hover:bg-blue-50' : 'border-white/50 text-white hover:bg-white/20']">
                                Masuk
                            </Link>
                        </template>
                    </div>
            ...
            >
                <div v-if="isMobileMenuOpen" class="md:hidden absolute top-[115%] left-0 right-0 bg-white/95 backdrop-blur-md border border-[#bfc9bd]/70 rounded-2xl shadow-xl p-4 flex flex-col gap-2.5">
                    ...
                    <div class="border-t border-[#bfc9bd]/30 pt-2.5 flex flex-col gap-2">
                        ...
                        <template v-else>
                            <Link :href="route('unit.login', { slug: 'ketahanan-pangan' })" class="w-full inline-flex items-center justify-center px-4 py-2 border border-blue-600 text-blue-700 font-bold text-xs rounded-xl hover:bg-blue-50 transition">
                                Masuk
                            </Link>
                        </template>
                    </div>
        ...
        <section class="relative pt-32 pb-24 md:pb-32 overflow-hidden min-h-screen flex items-center justify-center">
            ...
            <div class="max-w-4xl mx-auto px-6 text-center relative z-10 space-y-4 md:space-y-6 -mt-4 md:-mt-8">
                <h2 class="text-3xl md:text-5xl lg:text-6xl font-extrabold text-white leading-tight md:leading-tight tracking-tight drop-shadow-lg">
                    {{ (settings.hero_title || 'Unit Ketahanan Pangan BUMDES Dammar Wulan').replace(/BUMDesa|BUMDes/ig, 'BUMDES') }}
                </h2>
                
                <div class="flex items-start justify-center">
                    <p class="text-sm md:text-lg text-slate-200 max-w-2xl mx-auto leading-relaxed drop-shadow-md">
                        {{ settings.hero_subtitle || 'Solusi pengelolaan lumbung desa, bibit pertanian, pupuk bersubsidi, dan distribusi pangan demi kesejahteraan masyarakat.' }}
                    </p>
                </div>

                <div class="pt-8 flex flex-col sm:flex-row items-center justify-center gap-4">
                    <Link :href="route('unit.login', { slug: 'ketahanan-pangan' })" class="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-8 py-3.5 bg-blue-600 hover:bg-blue-700 text-white font-bold text-sm rounded-xl shadow-lg hover:shadow-blue-600/30 hover:-translate-y-0.5 transition-all">
            ...
            <div class="max-w-5xl mx-auto px-6 grid gap-8 md:grid-cols-2 items-center">
                <div class="space-y-5 scroll-animate scroll-slide-right">
                    ...
                    <h3 class="text-2xl font-bold text-slate-800 leading-tight">
                        {{ settings.about_title || 'Tentang Unit Ketahanan Pangan' }}
                    </h3>
                    <p class="text-xs md:text-sm text-[#404940] leading-relaxed">
                        {{ (settings.about_description || 'Unit Ketahanan Pangan adalah pilar penting dalam BUMDES Dammar Wulan, fokus pada pengelolaan sumber daya pangan desa, pertanian terpadu, serta pemenuhan kebutuhan pokok untuk memastikan stabilitas harga dan kemandirian pangan masyarakat.').replace(/BUMDesa|BUMDes/ig, 'BUMDES') }}
                    </p>
                ...
                <div class="scroll-animate scroll-scale-in relative bg-gradient-to-br from-blue-50 to-[#ffffff] p-8 border border-[#bfc9bd] rounded-2xl shadow-sm flex flex-col items-center justify-center text-center overflow-hidden">
                    ...
                    <h4 class="font-extrabold text-slate-800 text-base uppercase tracking-wider">Ketahanan Pangan</h4>
                    ...
                    <p class="text-xs text-[#404940] leading-relaxed max-w-sm">
                        Membangun kemandirian pangan desa untuk masa depan yang lebih baik.
                    </p>
                </div>
            </div>
        </section>
        ...
        <section id="layanan" class="py-20 bg-slate-50 border-b border-[#bfc9bd]">
            <div class="max-w-5xl mx-auto px-6 space-y-12">
                <div class="text-center space-y-3 scroll-animate scroll-fade-up">
                    ...
                    <h3 class="text-2xl font-bold text-slate-800">Layanan Pertanian & Pangan</h3>
                    <p class="text-xs text-[#404940] max-w-md mx-auto">Berbagai dukungan penyediaan bahan pangan pokok, bibit, serta lumbung desa.</p>
                </div>
                ...
                <div class="grid gap-6 md:grid-cols-3">
                    ...
                    <div class="scroll-animate scroll-scale-in bg-white border border-[#bfc9bd] p-6 rounded-xl hover:border-blue-500 hover:shadow-md transition-all duration-300 flex flex-col justify-between">
                        <div class="space-y-4">
                            ...
                            <div>
                                <h4 class="font-bold text-slate-800 text-sm mb-1.5">{{ settings.feature_1_title || 'Distribusi Pupuk & Bibit' }}</h4>
                                <p class="text-[11px] text-[#5c5f61] leading-relaxed">{{ settings.feature_1_desc || 'Layanan distribusi saprotan untuk meningkatkan hasil panen warga.' }}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="scroll-animate scroll-scale-in bg-white border border-[#bfc9bd] p-6 rounded-xl hover:border-blue-500 hover:shadow-md transition-all duration-300 flex flex-col justify-between" style="transition-delay: 80ms">
                        <div class="space-y-4">
                            ...
                            <div>
                                <h4 class="font-bold text-slate-800 text-sm mb-1.5">{{ settings.feature_2_title || 'Lumbung Desa' }}</h4>
                                <p class="text-[11px] text-[#5c5f61] leading-relaxed">{{ settings.feature_2_desc || 'Tempat penyimpanan cadangan pangan untuk menjaga stabilitas ketersediaan saat paceklik.' }}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="scroll-animate scroll-scale-in bg-white border border-[#bfc9bd] p-6 rounded-xl hover:border-blue-500 hover:shadow-md transition-all duration-300 flex flex-col justify-between" style="transition-delay: 160ms">
                        <div class="space-y-4">
                            ...
                            <div>
                                <h4 class="font-bold text-slate-800 text-sm mb-1.5">{{ settings.feature_3_title || 'Pemasaran Hasil Tani' }}</h4>
                                <p class="text-[11px] text-[#5c5f61] leading-relaxed">{{ settings.feature_3_desc || 'Membantu petani memasarkan hasil panen dengan harga terbaik langsung ke pasar induk.' }}</p>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <section id="struktur" class="py-20 bg-white border-b border-[#bfc9bd]">
            <div class="max-w-6xl mx-auto px-6">
                <div class="text-center space-y-3 mb-16 scroll-animate scroll-fade-up">
                    ...
                    <h3 class="text-2xl font-bold text-slate-800">Struktur Organisasi Unit Ketahanan Pangan</h3>
                    <p class="text-xs text-[#404940] max-w-md mx-auto">Manajemen pengelola Unit Ketahanan Pangan BUMDES Dammar Wulan.</p>
                </div>
                ...
                <div class="hidden md:block overflow-x-auto pb-4">
                    <div class="relative min-w-[600px] max-w-3xl mx-auto">
                        ...
                        <!-- Level 2: Kepala Unit Ketahanan Pangan -->
                            ...
                            <div class="org-node flex flex-col items-center scroll-animate scroll-fade-up" style="transition-delay: 200ms">
                                <div class="relative group cursor-default">
                                    <div class="w-20 h-20 rounded-full border-4 border-blue-600 bg-gradient-to-br from-blue-500 to-blue-700 flex items-center justify-center shadow-lg text-white font-extrabold text-xl ring-4 ring-blue-500/20 group-hover:scale-105 transition-all duration-300 overflow-hidden">
                                        <img v-if="settings.org_unit_kp_image" :src="settings.org_unit_kp_image" class="w-full h-full object-cover" />
                                        <span v-else>{{ getInitials(settings.org_unit_kp_name || 'Kepala KP') }}</span>
                                    </div>
                                </div>
                                <div class="mt-3 text-center">
                                    <p class="text-xs font-extrabold text-slate-800">{{ settings.org_unit_kp_name || 'Kepala Unit KP' }}</p>
                                    <span class="mt-1 inline-block px-2.5 py-0.5 bg-blue-600 text-white text-[9px] font-bold uppercase tracking-wider rounded-full">Kepala Unit Ketahanan Pangan</span>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="org-node flex flex-col items-center scroll-animate scroll-fade-up" style="transition-delay: 350ms">
                                <div class="relative group cursor-default">
                                    <div class="w-16 h-16 rounded-full border-4 border-blue-500 bg-gradient-to-br from-blue-400 to-blue-600 flex items-center justify-center shadow text-white font-extrabold text-lg ring-4 ring-blue-500/10 group-hover:scale-105 transition-all duration-300 overflow-hidden">
                                        <img v-if="settings.org_unit_kp_staff_image" :src="settings.org_unit_kp_staff_image" class="w-full h-full object-cover" />
                                        <span v-else>{{ getInitials(settings.org_unit_kp_staff_name || 'Staf KP') }}</span>
                                    </div>
                                </div>
                                <div class="mt-2 text-center">
                                    <p class="text-xs font-bold text-slate-800">{{ settings.org_unit_kp_staff_name || 'Nama Staf' }}</p>
                                    <span class="mt-0.5 inline-block px-2 py-0.5 bg-blue-100 text-blue-700 text-[8px] font-bold uppercase tracking-wider rounded-full">Staf Unit KP</span>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="md:hidden space-y-6">
                    ...
                    <div class="flex flex-col items-center">
                        ...
                        <div class="bg-blue-50/50 border border-blue-100 rounded-2xl p-4 flex items-center gap-3 w-full max-w-[280px]">
                            <div class="w-12 h-12 rounded-full border-2 border-blue-600 overflow-hidden shrink-0 shadow-sm bg-gradient-to-br from-blue-500 to-blue-700 flex items-center justify-center text-white font-bold text-xs">
                                <img v-if="settings.org_unit_kp_image" :src="settings.org_unit_kp_image" class="w-full h-full object-cover" />
                                <span v-else>{{ getInitials(settings.org_unit_kp_name || 'Kepala Unit') }}</span>
                            </div>
                            <div>
                                <h4 class="text-xs font-extrabold text-slate-800">{{ settings.org_unit_kp_name || 'Kepala Unit' }}</h4>
                                <p class="text-[9px] font-bold text-blue-700 mt-0.5">Ka. Unit Pangan</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="flex flex-col items-center">
                        ...
                        <div class="bg-blue-50/50 border border-blue-100 rounded-2xl p-4 flex items-center gap-3 w-full max-w-[280px]">
                            ...
                            <div>
                                <h4 class="text-xs font-extrabold text-slate-800">{{ settings.org_unit_kp_staff_name || 'Nama Staf' }}</h4>
                                <p class="text-[9px] font-bold text-blue-700 mt-0.5">Staf Unit Pangan</p>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <section id="galeri" class="py-20 bg-slate-50 border-b border-[#bfc9bd]">
            <div class="max-w-6xl mx-auto px-6 space-y-12">
                <div class="text-center space-y-3 scroll-animate scroll-fade-up">
                    ...
                    <h3 class="text-2xl font-bold text-slate-800">Galeri Unit Ketahanan Pangan</h3>
                    <p class="text-xs text-[#404940] max-w-md mx-auto">Dokumentasi kegiatan dan aktivitas pelayanan Unit Ketahanan Pangan BUMDES Dammar Wulan.</p>
                </div>
        ...
        <section id="berita" class="py-20 bg-[#f8fafc] border-b border-[#bfc9bd]">
            <div class="max-w-5xl mx-auto px-6 space-y-12">
                <div class="text-center space-y-3 scroll-animate scroll-fade-up">
                    ...
                    <p class="text-xs text-[#404940] max-w-md mx-auto">Informasi terbaru seputar kegiatan Unit Ketahanan Pangan.</p>
                </div>
        ...
        <section id="faq" class="py-20 bg-[#ffffff] border-b border-[#bfc9bd]">
            <div class="max-w-3xl mx-auto px-6 space-y-12">
                <div class="text-center space-y-3 scroll-animate scroll-fade-up">
                    ...
                    <p class="text-xs text-[#404940] max-w-md mx-auto">Temukan jawaban atas beberapa pertanyaan umum seputar Ketahanan Pangan.</p>
                </div>
        ...
        <section id="kontak" class="py-20 bg-slate-900 text-slate-105 relative overflow-hidden">
            ...
            <div class="max-w-4xl mx-auto px-6 relative z-10 grid gap-8 md:grid-cols-2">
                <div class="space-y-4 scroll-animate scroll-slide-right">
                    <h3 class="text-xl font-bold text-white">Hubungi Unit Ketahanan Pangan</h3>
                    <p class="text-xs text-slate-400 leading-relaxed">Punya pertanyaan tentang jadwal distribusi bibit atau ketersediaan bahan pangan? Tim kami siap membantu Anda.</p>
                    
                    <div class="space-y-3 pt-2 text-xs">
                        <div class="flex items-center gap-2">
                            <span class="material-symbols-outlined text-blue-500 text-[18px]">location_on</span>
                            <span class="text-slate-300">{{ (settings.contact_address || 'Lumbung Pangan, Desa Ciwuni').replace(/BUMDesa|BUMDes/ig, 'BUMDES') }}</span>
                        </div>
                        <div class="flex items-center gap-2">
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <span class="text-slate-300">{{ settings.contact_email || 'pangan@ciwuni.desa.id' }}</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="scroll-animate scroll-slide-left bg-white/5 border border-white/10 p-6 rounded-xl flex flex-col justify-between">
                    <div>
                        <h4 class="font-bold text-sm text-white mb-2">Jam Operasional Pelayanan</h4>
        ...
        <footer class="py-8 bg-slate-950 text-center text-slate-550 border-t border-slate-900 text-xs">
            <div class="max-w-7xl mx-auto px-6 flex flex-col md:flex-row items-center justify-between gap-4">
                <p>&copy; 2026 BUMDes Dammar Wulan &bull; Ketahanan Pangan. Hak Cipta Dilindungi.</p>
                <p class="text-[10px] text-slate-600">Sistem Informasi Pertanian & Pangan &bull; Desa Ciwuni</p>
            </div>
        </footer>
    </div>
</template>
