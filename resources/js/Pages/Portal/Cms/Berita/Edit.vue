...
const form = useForm({
    ...
    thumbnail: props.post.thumbnail || null,
...
const previewThumbnail = ref(props.post.thumbnail || null);
...
const submit = () => {
    form.post(route('portal.cms.berita.update', props.post.id));
};
</script>
...
<template>
    ...
    <PortalLayout>
        <template #header>Edit Berita</template>
        <div class="max-w-3xl space-y-6">
            <div class="flex items-center gap-2">
                <Link :href="route('portal.cms.berita.index')" class="text-sm text-slate-500 hover:text-emerald-700 transition">← Kembali</Link>
            </div>
            ...
            <form @submit.prevent="submit" class="space-y-6">
                <div class="bg-white border border-slate-200 rounded-xl p-6 space-y-4 shadow-sm">
                    <div>
                        ...
                        <input v-model="form.judul" type="text" class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition" />
                    ...
                    <div>
                        ...
                        <textarea v-model="form.ringkasan" rows="2" class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition"></textarea>
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-slate-700 mb-1">Konten *</label>
                        <textarea v-model="form.konten" rows="10" class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition"></textarea>
                    ...
                    <div class="grid gap-4 md:grid-cols-2">
                        ...
                        <div>
                            ...
                            <input v-model="form.published_at" type="datetime-local" class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition" />
                        </div>
                    </div>
                </div>
                ...
                <div class="flex justify-end gap-3">
                    ...
                    <button type="submit" :disabled="form.processing" class="inline-flex items-center gap-2 px-6 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white font-semibold text-sm rounded-lg shadow-sm transition disabled:opacity-50">
                        ...
                        {{ form.processing ? 'Menyimpan...' : 'Perbarui Berita' }}
                    </button>
                </div>
            </form>
        </div>
