...
<head>
    ...
    <title>Laporan Detail Database Pelanggan WiFi — BUMDes Ciwuni</title>
...
<body>
    ...
    <div class="no-print-bar">
        <div>
            <strong>Preview Cetak Laporan Rincian Pelanggan WiFi</strong>
    ...
    <div class="kop-surat">
        ...
        <div class="kop-meta">
            <p>Dokumen: <strong>DATABASE PELANGGAN RINCI</strong></p>
            <p>Tanggal Cetak: <strong>{{ date('d F Y, H:i') }} WIB</strong></p>
    ...
    <div class="summary-grid">
        ...
        <div class="summary-card">
            ...
            <strong>Rp {{ number_format($stats['total_tarikan'], 0, ',', '.') }}</strong>
        </div>
        ...
        <div class="summary-card blue">
            <span>Hak Provider ISP</span>
            <strong>Rp {{ number_format($stats['total_provider'], 0, ',', '.') }}</strong>
        </div>
    </div>

    <!-- TABEL DATA PELANGGAN DETAIL -->
    <table class="data-table">
        <thead>
            <tr>
                <th class="text-center" style="width:25px;">No</th>
                <th style="width:85px;">ID Pelanggan</th>
                <th>Nama Pelanggan &amp; NIK</th>
                <th style="width:90px;">No WhatsApp</th>
                <th>Alamat &amp; RT/RW</th>
                <th>Provider / ISP</th>
                <th style="width:75px;">Paket Speed</th>
                <th class="text-right" style="width:80px;">Tarikan (Rp)</th>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td class="text-center">
                    @if($p->provider && $p->provider->tipe_bagi_hasil === 'FLAT_ADMIN')
                        <span class="badge badge-flat">FLAT</span>
                    @else
                        <span class="badge badge-pct">{{ $p->bagi_hasil_bumdes ?? 9 }}%</span>
                    @endif
                </td>
        ...
        <tfoot>
            <tr style="background-color:#f1f5f9; font-weight:bold;">
                <td colspan="7" class="text-right uppercase" style="padding:6px;">TOTAL KESELURUHAN:</td>
                <td class="text-right font-mono font-bold">Rp {{ number_format($stats['total_tarikan'], 0, ',', '.') }}</td>
                <td></td>
                <td class="text-right font-mono font-bold" style="color:#047857;">Rp {{ number_format($stats['total_hasil_bumdes'], 0, ',', '.') }}</td>
                <td class="text-right font-mono font-bold" style="color:#1d4ed8;">Rp {{ number_format($stats['total_provider'], 0, ',', '.') }}</td>
                <td colspan="3"></td>
            </tr>
        </tfoot>
    </table>
